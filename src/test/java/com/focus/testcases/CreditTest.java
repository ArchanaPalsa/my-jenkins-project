...
public class CreditTest extends BaseEngine 
{
     ...
     @Test(priority=96)
		public void checkRestoreCompanyForP1() throws EncryptedDocumentException, InvalidFormatException, IOException, InterruptedException, AWTException
		{
			cp = new CreditPage(getDriver());
			Assert.assertEquals(cp.checkRestoreCompany(), true);
		}
	...
	// @Test(priority=182)
		public void checkLoginAfterRestoringBackup() throws EncryptedDocumentException, InvalidFormatException, IOException, InterruptedException
		{
			cp = new CreditPage(getDriver());
			Assert.assertEquals(cp.checkLogin(), true);
		}
	...
	// @Test(priority=217)//no need
	 ...
	 @Test(priority=400)
	  ...
	  {
		 cp=new CreditPage(getDriver());
		  Assert.assertEquals(cp.checklogout(), true);
	  }
